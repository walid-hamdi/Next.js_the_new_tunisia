...
export const PeerContextProvider = ({ children, initialContext }) => {
  ...
  const [peerListenersInitialized, setPeerListenersInitialized] =
    useState(false);
  ...
  const [roomMetadata, setRoomMetadata, roomMetadataRef] =
    useStateRef(initialRoomMetadata);
  ...
  function onSpeakerClosesStream(conn) {
    try {
      ...
      incomingStreamsRef.current
        .filter((c) => c.call.peer !== conn.peer)
        .audioObj.pause();
      setIncomingStreams([
        ...incomingStreamsRef.current.filter((c) => c.call.peer !== conn.peer),
      ]);
    } catch (e) {
      // TODO: Handle error
    }
  ...
  function closedStreamToPeer(call) {
    ...
    call.close();
    setOutgoingStreams([
      ...outgoingStreamsRef.current.filter((c) => c.peer !== call.peer),
    ]);
  }
