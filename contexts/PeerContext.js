...
const PeerContextProvider = ({ children, initialContext }) => {
  ...
  const [
    incomingStreamsAudio,
    setIncomingStreamsAudio,
    incomingStreamsAudioRef,
  ] = useStateRef([]);
  const [incomingStreamsObj, setIncomingStreamsObj, incomingStreamsObjRef] =
    useStateRef([]);
  ...
  const [connRole, setConnRole, connRoleRef] = useStateRef(
  ...
  const { startMicStream } = useContext(StreamContext);
  ...
  function onDisconnectedPeer(conn) {
    return setConnectedPeers(
      connectedPeersRef.current.filter((peer) => peer.peer !== conn.peer)
    );
  }

  // HOST: Ask peer to be Speaker
  function onPromotePeerToSpeaker(peerId) {
    const willPromoteConn = connectedPeersRef.current.find(
      ({ peer }) => peer === peerId
    );
  ...
  function startStreamToAllPeers() {
    ...
    peersToCall.forEach(async (peer, i) => {
  ...
  function onSpeakerClosesStream(conn, e) {
    ...
    setIncomingStreams([
      ...incomingStreamsRef.current.filter((c) => c.peer !== conn.peer),
    ]);
    setIncomingStreamsObj([
      ...incomingStreamsObjRef.current.filter((c) => c.call.peer !== conn.peer),
    ]);
    // debugger;
    // a
  ...
  function startRoom() {
    ...
    peer.on("call", (call) => {
      ...
      call.on("close", (e) => {
        // console.log("Close call", e);
        onSpeakerClosesStream(call, e);
      });
      call.on("error", (e) => {
      ...
      call.on("stream", (audioStream) => {
        setIncomingStreamsAudio([
          ...incomingStreamsAudioRef.current,
          audioStream,
        ]);
